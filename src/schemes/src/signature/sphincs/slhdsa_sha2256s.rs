...
use mikomikagi_core::keys::{
    EncryptionParameters, Fingerprint, SignaturePrivateKey, SignaturePublicKey, AES256GCM,
    CHACHA20_POLY1305,
};
use rand::{rngs::StdRng, SeedableRng};
use slh_dsa::{
    signature::{Keypair, SignerMut, Verifier}, Sha2_256s, Signature, SigningKey, VerifyingKey
};
...
use super::super::{GenericSignaturePrivateKey, GenericSignaturePublicKey, SignatureScheme};

// ---------------------------------- Defintions --------------------------------------

pub struct SlhDsaSha2256s;

#[derive(Clone)]
#[repr(transparent)]
/// Implementation wrapper over VerifyingKey<Sha2_256s>
pub struct PublicKey(VerifyingKey<Sha2_256s>);

#[derive(Clone)]
#[repr(transparent)]
/// Implementation wrapper over SigningKey<Sha2_256s>
pub struct PrivateKey(SigningKey<Sha2_256s>);
...
impl GenericSignaturePublicKey for PublicKey {
    fn serialize(self, owner_name: &str) -> Result<SignaturePublicKey, Error> {
        ...
        let fingerprint =
            *blake3::keyed_hash(blake3::hash(owner_name.as_bytes()).as_bytes(), &bytes).as_bytes();

        Ok(SignaturePublicKey::new(
            SlhDsaSha2256s::SCHEME_CODE,
            fingerprint.into(),
            bytes,
            None,
        ))
    ...
    fn deserialize(pk: &SignaturePublicKey) -> Result<Self, Error> {
        VerifyingKey::<Sha2_256s>::try_from(pk.bytes())
            .map(PublicKey)
...
impl GenericSignaturePrivateKey for PrivateKey {
    ...
    fn deserialize(sk: &SignaturePrivateKey, key: Option<&[u8]>) -> Result<Self, Error> {
        assert!(
            sk.encryption.is_some() == key.is_some(),
            "Attempted to deserialize an encrypted key without the decryption key"
        );

        match &sk.encryption {
            None => {
                let private_key = SigningKey::<Sha2_256s>::try_from(sk.bytes.as_ref())
                    .map_err(|_| Error::SerializationFailed(SerializationError::RustCrypto))?;
                ...
                match *algorithm {
                    AES256GCM => {
                        ...
                        let plaintext = cipher
                            .decrypt(&nonce.into(), sk.bytes.as_ref())
                            .map_err(|_| Error::DecryptionFailed)?;

                        let private_key = SigningKey::<Sha2_256s>::try_from(plaintext.as_ref())
                            .map_err(|_| {
                                Error::SerializationFailed(SerializationError::RustCrypto)
                            })?;
...
impl SignatureScheme for SlhDsaSha2256s {
    ...
    type Signature = Signature<Sha2_256s>;

    type Error = Error;

    fn keypair() -> (Self::PublicKey, Self::PrivateKey) {
        let mut rng = StdRng::from_entropy();
        let sk = slh_dsa::SigningKey::<Sha2_256s>::new(&mut rng);
    ...
    fn verify(message: &[u8], signature: &Self::Signature, pk: &Self::PublicKey) -> bool {
        VerifyingKey::<Sha2_256s>::verify(pk, message, signature).is_ok()
    }
}

impl Deref for PublicKey {
    type Target = VerifyingKey<Sha2_256s>;
...
impl Deref for PrivateKey {
    type Target = SigningKey<Sha2_256s>;
...
impl Parseable for Signature<Sha2_256s> {
