...
use mikomikagi_schemes::{encryption::{kyber::{MlKem1024, MlKem768}, EncryptionScheme, GenericEncapsulationPrivateKey, GenericEncapsulationPublicKey}, error::Error, signature::{dilithium::{MlDsa44, MlDsa65, MlDsa87}, sphincs::{SlhDsaSha2128s, SlhDsaSha2256s}, GenericSignaturePrivateKey, GenericSignaturePublicKey, SignatureScheme}, utils::EncryptionArguments};
...
impl<'a> StandaloneKeypairBuilder<'a> {
    ...
    pub fn build(&self) -> Result<KeygenResult,Error> {
        ...
        let (pk,mut sk) = (match self.signature_scheme.expect("No signature scheme set in builder") {
            ...
            SlhDsaSha2256s::SCHEME_CODE => Self::signature_genkey::<SlhDsaSha2256s>,
            MlDsa44::SCHEME_CODE => Self::signature_genkey::<MlDsa44>,
            MlDsa65::SCHEME_CODE => Self::signature_genkey::<MlDsa65>,
            MlDsa87::SCHEME_CODE => Self::signature_genkey::<MlDsa87>,
        ...
        let (ek,dk) = match self.encryption_scheme {
            ...
            Some(s) => {
                let (ek,dk) = (match s {
                    MlKem768::SCHEME_CODE => Self::encryption_genkey::<MlKem768>,
                    MlKem1024::SCHEME_CODE => Self::encryption_genkey::<MlKem1024>,
