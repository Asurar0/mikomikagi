...
use mikomikagi_schemes::{encryption::{kyber::{MlKem1024, MlKem768}, EncryptionScheme, GenericEncapsulationPrivateKey, GenericEncapsulationPublicKey}, error::Error, utils::Parseable};
...
impl<'dk> DecapsulationBuilder<'dk> {
    ...
    pub fn decapsulate(&self, ciphertext: &[u8]) -> Result<Vec<u8>,Error> {
        ...
        match self.dk.scheme() {
            MlKem768::SCHEME_CODE => Self::decapsulate_scheme::<MlKem768>(ciphertext, self.dk, self.key),
            MlKem1024::SCHEME_CODE => Self::decapsulate_scheme::<MlKem1024>(ciphertext, self.dk, self.key),
...
impl<'ek> EncapsulationBuilder<'ek> {
    ...
    pub fn encapsulate(&self) -> Result<(Vec<u8>,Vec<u8>),Error> {
        ...
        match self.ek.scheme {
            MlKem768::SCHEME_CODE => Self::encapsulate_scheme::<MlKem768>(self.ek),
            MlKem1024::SCHEME_CODE => Self::encapsulate_scheme::<MlKem1024>(self.ek),
            _ => panic!("Unknown Signature Scheme!")
        }
    }
